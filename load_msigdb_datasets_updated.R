...
# KEGG Legacy pathway gene sets
an_df_kegg_legacy <- msigdbr(species = "Homo sapiens", category = "C2", subcategory = "CP:KEGG_LEGACY") %>% 
  dplyr::select(gs_name, gene_symbol) %>% 
  as.data.frame()

# KEGG medicus pathway gene sets
an_df_kegg_medicus <- msigdbr(species = "Homo sapiens", category = "C2", subcategory = "KEGG_MEDICUS") %>% 
  dplyr::select(gs_name, gene_symbol) %>% 
  as.data.frame()
...
gene_annodf_list <- list(
  ...
  kegg_legacy = an_df_kegg_legacy,
  kegg_medicus = an_df_kegg_medicus,
